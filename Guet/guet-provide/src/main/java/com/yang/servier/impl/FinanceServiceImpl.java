...
public class FinanceServiceImpl implements IFinanceService {
    ...
    @Override
    public List<FinanceVO> getFinanceList(Long userId, Integer paymentStatus, String keyword) {
        List<FinanceVO> list = financeMapper.selectFinanceList(userId, paymentStatus, keyword);
    ...
    public void addPayment(PaymentDTO dto) {
        ...
        // 3. 校验付款金额是否超过待付金额
        if (dto.getPaymentAmount().compareTo(finance.getUnpaidAmount()) > 0) {
            throw new RuntimeException("付款金额不能超过待付金额");
        }
    ...
    public FinanceVO getFinanceDetail(Long id) {
        FinanceVO vo = financeMapper.selectFinanceDetail(id);
        if (vo != null) {
            vo.setPaymentStatusText(getStatusText(vo.getPaymentStatus()));
        }
        return vo;
    ...
    @Override
    public Finance getFinanceByOrderId(Integer orderId) {
        return financeMapper.selectByOrderId(orderId);
    }
