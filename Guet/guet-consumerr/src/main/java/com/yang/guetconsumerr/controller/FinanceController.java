...
public class FinanceController {
    ...
    public Result<Map<String, Object>> getFinanceList(@RequestParam(required = false) Long userId,
        ...
        Map<String, Object> data = financeService.getFinanceList(userId, paymentStatus, keyword);
    ...
    // http://localhost:8088/consumer/api/finance/order/{orderId}
    @GetMapping("/order/{orderId}")
    public Result<Finance> getFinanceByOrderId(@PathVariable Integer orderId) {
        Finance finance = financeService.getFinanceByOrderId(orderId);
        return Result.build(finance, 200, "查询成功");
    }
}
